...
class Hyperparameters:
    ...
    mamba_layers = int(os.environ.get("MAMBA_LAYERS", "0"))  # Number of early layers to replace with Mamba
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # Use SDPA (B,H,T,D layout) - flash backend is auto-selected on H100
        ...
        y = F.scaled_dot_product_attention(q_sdpa, k_sdpa, v_sdpa, attn_mask=None, is_causal=True)
...
class MambaBlock(nn.Module):
    """Gated causal convolution block with SiLU² activation, matching MLP style.
    Replaces attention with depthwise causal conv + gating. Fast and compilable."""
    def __init__(self, dim: int, state_dim: int = 16, conv_width: int = 4, expand: int = 2, layer_idx: int = 0, ln_scale: bool = False):
        super().__init__()
        self.dim = dim
        self.inner = dim * expand
        self.norm = RMSNorm()
        self.ln_scale_factor = 1.0 / math.sqrt(layer_idx + 1) if ln_scale else 1.0
        # In projection: value + gate
        self.in_proj = CastedLinear(dim, self.inner * 2, bias=False)
        # Single causal depthwise conv (captures local patterns)
        self.conv = nn.Conv1d(self.inner, self.inner, 16, padding=15, groups=self.inner, bias=True)
        # Output projection (zero-init)
        self.out_proj = CastedLinear(self.inner, dim, bias=False)
        self.resid_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        nn.init.zeros_(self.out_proj.weight)

    def forward(self, x: Tensor) -> Tensor:
        residual = x
        x = self.norm(x) * self.ln_scale_factor
        B, T, D = x.shape
        xz = self.in_proj(x)
        x_branch, z = xz.split([self.inner, self.inner], dim=-1)
        # Causal depthwise conv
        y = self.conv(x_branch.transpose(1, 2))[..., :T].transpose(1, 2)
        y = F.silu(y) * F.silu(z)  # gated activation
        out = self.out_proj(y)
        return residual + self.resid_scale.to(dtype=out.dtype)[None, None, :] * out
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mamba_layer_count = mamba_layers
        self.mamba_layer_set = set(range(mamba_layers))
        num_transformer_layers = num_layers - mamba_layers
        ...
        # Mamba blocks for early layers (replace attention+MLP with selective SSM)
        self.mamba_blocks = nn.ModuleList(
            [MambaBlock(model_dim, state_dim=mamba_state_dim, expand=mamba_expand,
                       layer_idx=i, ln_scale=ln_scale) for i in range(mamba_layers)]
        ) if mamba_layers > 0 else nn.ModuleList()
        if rope_dims > 0:
            head_dim = model_dim // num_heads
    ...
    def _run_layers(self, x: Tensor, x0: Tensor, input_ids: Tensor) -> Tensor:
        """Shared layer execution for both forward and forward_logits."""
        n = self.num_layers
        v0 = None
        skips: list[Tensor] = []
        ve_cache: dict = {}
        ...
        for i in range(self.num_decoder_layers):
            ...
            if bi in self.mamba_layer_set:
                x = self.mamba_blocks[bi](x)
            else:
...
def main() -> None:
    ...
    use_fullgraph = args.mamba_layers == 0  # Mamba scan breaks fullgraph
    ...
    # Mamba block parameters -> scalar optimizer (Adam)
    ...
    eval_model = GPT(
        ...
        mamba_layers=args.mamba_layers, mamba_state_dim=args.mamba_state_dim, mamba_expand=args.mamba_expand,
    ).to(device).bfloat16()
