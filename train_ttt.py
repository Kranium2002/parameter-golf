...
try:
    ...
    flash_attn_3_func = None  # Will use SDPA fallback
class Hyperparameters:
    ...
    int4_mlp_qat = bool(int(os.environ.get("INT4_MLP_QAT", "0")))  # Int4 QAT for MLP weights
    ...
    ttt_warmup_epochs = int(os.environ.get("TTT_WARMUP_EPOCHS", 0))  # warmup within chunk
    ttt_val_chunks = int(os.environ.get("TTT_VAL_CHUNKS", 0))  # 0=all, N=first N chunks
...
def _int4_ste(w: Tensor) -> Tensor:
    """STE int4 fake-quantization for MLP weights."""
    with torch.no_grad():
        w32 = w.float()
        row_max = w32.abs().amax(dim=-1)
...
class MLP(nn.Module):
    _int4_qat: bool = False
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        # No CastedLinear -- weights come from banks
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        if MLP._int4_qat and self.training:
            up_w = _int4_ste(up_w)
            down_w = _int4_ste(down_w)
...
def eval_val_sliding_ttt(
    ...
    # Build optimizer with optional per-layer LR scaling
    if args.ttt_per_layer_lr:
        ...
        for name, p in base_model.named_parameters():
            if not p.requires_grad:
                continue
            # Scale LR: deeper layers get higher LR (1x to 2x)
            layer_scale = 1.0
            for bi in range(num_blocks):
                if f"blocks.{bi}." in name or (name.startswith("qo_bank") or name.startswith("kv_bank") or name.startswith("mlp_")):
                    layer_scale = 1.0 + bi / num_blocks
                    break
            param_groups.append({"params": [p], "lr": args.ttt_lr * layer_scale})
    else:
        param_groups = [{"params": ttt_params, "lr": args.ttt_lr}]

    if args.ttt_optimizer == "adamw":
        optimizer = torch.optim.AdamW(param_groups, betas=(0.9, 0.99), weight_decay=0.01)
        log0(f"ttt_sliding:optimizer=AdamW betas=(0.9,0.99) wd=0.01")
    else:
        optimizer = torch.optim.SGD(param_groups, lr=args.ttt_lr, momentum=args.ttt_momentum)
        log0(f"ttt_sliding:optimizer=SGD momentum={args.ttt_momentum}")
    t0 = time.perf_counter()
    effective_num_chunks = args.ttt_val_chunks if args.ttt_val_chunks > 0 else num_chunks

    for ci in range(effective_num_chunks):
        ...
        is_last_chunk = (ci == effective_num_chunks - 1)
        if not is_last_chunk and args.ttt_epochs > 0:
            base_model.train()
            chunk_seqs = (chunk_end - chunk_start) // seq_len
            if chunk_seqs > 0:
                cos_lr = args.ttt_lr * 0.5 * (1.0 + math.cos(math.pi * ci / max(effective_num_chunks - 1, 1)))
                for pg in optimizer.param_groups:
                    base_scale = pg.get('_lr_scale', 1.0)
                    pg['lr'] = cos_lr * base_scale
                    if '_lr_scale' not in pg:
                        pg['_lr_scale'] = pg['lr'] / max(cos_lr, 1e-9)
                my_seq_s = (chunk_seqs * rank) // world_size
                my_seq_e = (chunk_seqs * (rank + 1)) // world_size
                my_chunk_seqs = my_seq_e - my_seq_s
                for _ep in range(args.ttt_epochs):
                    # Warmup within chunk: first warmup_epochs at reduced LR
                    if args.ttt_warmup_epochs > 0 and _ep < args.ttt_warmup_epochs:
                        warmup_scale = 0.3 + 0.7 * (_ep / args.ttt_warmup_epochs)
                        for pg in optimizer.param_groups:
...
def quantize_int4_per_row(t: Tensor, clip_range: int = 7) -> tuple[Tensor, Tensor]:
    """Int4 quantization with GPTQ-lite clip search. Uses int8 storage."""
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = None, None, float('inf')
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
...
def main() -> None:
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            if args.int4_mlp_qat:
                MLP._int4_qat = True
                log0(f"late_qat:enabled step:{step} scale:{scale:.4f} int4_mlp=True")
            else:
    ...
    if args.smoke_test:
        log0("SMOKE_TEST: skipping EMA/quantization/sliding-window eval")
        if distributed:
    ...
    int4_cats = {"mlp"} if args.int4_mlp_qat else None
    int6_cats = {"attn"} if args.int4_mlp_qat else {"mlp", "attn"}
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, int6_cats, int4_cats=int4_cats)
